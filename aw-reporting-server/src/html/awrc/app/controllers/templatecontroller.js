(function() {
  ...
  /**
   * $routeParams.template_id: - none: we are creating a new template. - id: we
   * are editing an existing template. - +id: we are creating a new template
   * init'd with the contents of an existing one.
   */
  function TemplateController($scope, $location, $routeParams, $timeout,
      coreProvider) {
    ...
    if ($routeParams.template_id && $routeParams.template_id.length > 0) {
      ...
      coreProvider.getTemplateById(tid, function(err, template) {
        if (err) {
          ...
          $scope.load_error = "Unable to load the specified template ("
          ...
          if ($scope.based_on_existing) {
            ...
            $scope.template.templateName = "Copy of "
    ...
    $scope.saveTemplate = function() {
      $scope.waiting = true;
      coreProvider.saveTemplate($scope.template.id, $scope.template, function(
          err, tmpl) {
        if (err) {
          if (err.error == "missing_name")
            $scope.save_error = "You need to provide a name";
          else if (err.error == "missing_description")
            $scope.save_error = "You need to provide a description";
          else if (err.error == "missing_body")
            $scope.save_error = "You need to provide some template HTML";
          else
            $scope.save_error = "Unable to save template (" + err.error + ": "
                + err.message + ")";
          $scope.waiting = false;
        } else {
          $timeout(function() {
